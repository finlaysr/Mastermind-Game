    ...
    while swapped and n >= 0:  # Loop while not done moving elements around
        ...
        for i in range(0, n - 1):
            if (val(array[i]) > val(array[i + 1]) and not reverse) or (val(array[i]) < val(array[i + 1]) and reverse):
                ...
                swapped = True
        n -= 1  # Elements above n in list are now sorted since no need to go over them again
    return array
...
def binary_search(array: list | set | tuple, target: str | int | float) -> int | None:
    """Locate a value in a sorted array using binary search.
    Return index found at, None if not found"""
    low = 0; high = len(array)

    while low <= high:  # Repeat until value found or all values checked
    ...
    print('Target not found')  # If target not found
    return None
